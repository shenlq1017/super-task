...
`taskfile`（Taskfile 导入）、`importer`（README 导入）、`adopt`（孤儿进程纳管：发现结果 → generic 服务草稿，ipc.md §10.16）、`needs`（方向三·环境供给：声明式 needs 解析，resolve-only 四态：已存在/可安装/可归档供给/不可满足，ipc.md §10.17）、`toolchain` / `probe`（工具链探测、
