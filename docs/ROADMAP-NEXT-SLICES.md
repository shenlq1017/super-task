...
> J（隧道公网 URL 提取）、R（环境快照上下文）均已交付并移出。

| 顺位 | 切片 | 方向 | 调整后优先级 | 调整理由 |
|---|---|---|---|---|
| 1 | O 工作区定时备份 | 六 | 中 | 快照归档机制已就绪，只差定时触发与保留 |
| 2 | A 运行中进程原地接管 | 二 | 中 | 叙事核心，但 Windows attach 权限边界需先调研 |
| 3 | M 隧道模板并入现有工作区 | 四 | 中 | 复用 preview/apply；与 C 同批可做 |
| 4 | G compose 作为 needs 来源 | 三 | 中 | 依赖方向二数据复用边界拍板 |
| 5 | E 归档供给执行器 | 三 | 中 | 打开能力上限但工程量大（下载器/校验/隔离） |
| 6 | V M4 三平台真机冒烟 | 八 | 中 | 需真机环境；K 网关行为清单并入本项 |
| 7 | D2 钩子 / 级联重启 / 失败保持 | 一 | 中 | ROADMAP 主表剩余 ★★ 项，打包成一个切片 |
| 8 | B kind 智能推断 / X M6 差异收敛 / L apache 预检 | 二/八/四 | 中低 | 顺手级或依赖前项 |
| 9 | Q MCP 环境供给 | 七 | 中低 | 硬依赖 E 归档执行器落地 |
| 10 | S AI 审计回放 / N DB 感知备份 / H hosts / I 私有 CA / P 快照导出 | 七/六/四/六 | 低 | 高成本或主场景外 |
| 11 | U M3 签名公证 / W M5 AppImage / Y 模板远端分发 | 八/八/九 | 暂缓 | 需外部账号/市场；前置未齐 |
