...
> 验收方法总表见 `docs/VERIFICATION-MATRIX.md`（分层门禁 + 已交付回归 + 未交付验收 + 真机/发布清单）。
...
### M. 隧道模板并入现有工作区（替代独立工作区） —— ✅ 已交付（2026-09-06）

落地口径：`templates.mergePreview` / `templates.mergeApply`（`{ workspace_id,
template_id, source?, blocks?, ports?, params?, selected?, base_hash }`，写回走
saveForm）。组合模板复用 `plan_blocks`（依赖闭合 + `{{port}}` 占位），普通模板
取 params 替换后的 services 段；只增改所选，id/端口冲突与已存在文件跳过不覆盖；
`needs` 并集、`toolchain` 只补未钉扎键；来源留痕 `labels.origin=template-merge`；
幂等可重试。模板页详情与粘性操作条「并入当前工作区」入口。契约见
`docs/spec/ipc.md` §10.1。
