...
|---|---|---|---|---|
| 1 | **D 日志模式就绪判定** | 一 | **高 · 本轮执行** | 速览「高」行首项；Spring Boot 主打场景刚需；LogHub/健康线程材料齐备，成本可控 |
| 2 | T M2 release 平台产物 | 八 | **高**（P0） | M1 CI 已绿，发布产物是「对外承诺」的下一步；本地规划 M2=P0 |
| 3 | C Procfile 导入 | 二 | 中高 | 成本最小（格式极简）撬动 Overmind 用户池；preview/apply 模板现成 |
| 4 | F needs 钉扎写回 | 三 | 中高 | resolve 侧只读已交付，写回是纯增量小切片 |
| 5 | J 隧道公网 URL 提取 | 四 | 中 | 小成本高可感；复用日志管道 |
| 6 | R 环境快照上下文 | 七 | 中 | 聚合已有采样面，AI 价值高、无新权限 |
| 7 | O 工作区定时备份 | 六 | 中 | 快照归档机制已就绪，只差定时触发与保留 |
| 8 | A 运行中进程原地接管 | 二 | 中 | 叙事核心，但 Windows attach 权限边界需先调研 |
| 9 | M 隧道模板并入现有工作区 | 四 | 中 | 复用 preview/apply；与 C 同批可做 |
| 10 | G compose 作为 needs 来源 | 三 | 中 | 依赖方向二数据复用边界拍板 |
| 11 | E 归档供给执行器 | 三 | 中 | 打开能力上限但工程量大（下载器/校验/隔离） |
| 12 | V M4 三平台真机冒烟 | 八 | 中 | 需真机环境；K 网关行为清单并入本项 |
| 13 | B kind 智能推断 / X M6 差异收敛 / L apache 预检 | 二/八/四 | 中低 | 顺手级或依赖前项 |
| 14 | Q MCP 环境供给 | 七 | 中低 | 硬依赖 E 归档执行器落地 |
| 15 | S AI 审计回放 / N DB 感知备份 / H hosts / I 私有 CA / P 快照导出 | 七/六/四/六 | 低 | 高成本或主场景外 |
| 16 | U M3 签名公证 / W M5 AppImage / Y 模板远端分发 | 八/八/九 | 暂缓 | 需外部账号/市场；前置未齐 |
...
### D. 日志模式就绪判定 —— 优先级：高（本轮执行）

- **目标**：健康/就绪判定新增 `log` 来源（正则匹配如 `Started ... in ... seconds`），
  Spring Boot 主打场景不再只靠 tcp 猜。
- **验收雏形**：配置的服务只在满足就绪条件后报告 ready；超时 / 进程退出 / 日志
  未匹配 / 配置错误均有稳定结果；既有 `none/tcp/http` 与 restart 策略行为不变。
- **执行计划**（本轮，已按代码证据细化）：
  - Contract：`HealthType` 增 `log`；`HealthSpec` 增 `pattern`（清单加载校验：必填、
    ≤256 字符、正则可编译，违反 `SPEC_INVALID`）；**零新增错误码**。
  - Core：`health.rs` 增 `log_ready(pattern, lines)` 纯函数（单行匹配 + detail 片段
    截断）；引擎健康线程对 `log` 型改走 LogHub 扫描——Slot 增 `log_scan_seq` 水位
    （启动时 = 当前 `next_seq()`，天然排除上一轮进程的旧日志）+ `log_matched` 粘性
    命中（命中后不因环形缓冲淘汰回退）；每次扫描推进水位。
  - Adapter：yaml.md §6 健康字段表 + supertask.schema.json（enum 增 `log`、增
    `pattern`）；无 UI 改动（HealthView detail 既有展示直接生效）。
  - Verify：health/spec 单测 + 全量回归 + 三平台 CI。
- **边界**：环形缓冲淘汰窗口内到达的行可能漏扫（水位间隔 = 扫描间隔 2s，缓冲容量
  数千行，概率极低）；命中行 detail 截断 ≤120 字符（与日志页同内容面，无新增暴露）。
...
### C. Procfile 导入 —— 优先级：中高

- **目标**：`Procfile`（Overmind / Foreman / Heroku 生态）每行 `name: command`
  转成一个 generic 服务，撬动 Overmind 用户池；成本最小（格式极简）。
- **已有材料**：preview/apply 模板（Taskfile 导入同构：`preview(root, current)`
  纯函数 → 勾选 → `apply` 合并 → `save_form`）；`.env` 读取已有
...
### F. needs 安装与钉扎写回一体化（persist） —— 优先级：中高
...
### J. 隧道就绪信息：公网 URL 提取到服务卡片 —— 优先级：中

- **现状**：quick tunnel 的公网 URL（`*.trycloudflare.com`）只在服务日志里。
- **改进**：日志管道识别隧道分配 URL（cloudflared `https://<子域>.trycloudflare.com`
  行），提取后进运行页服务卡片/状态提示；只读展示、不回显 token。
- **待细化**：提取规则挂点（LogHub 管道 vs 快照时正则）、重启换 URL 的呈现、
  frpc `远程地址` 文案组装。
...
### O. 工作区定时备份与保留策略 —— 优先级：中

- **目标**：`data.volumes` 快照的定时自动创建与保留上限（条数/总字节/天数）。
- **已有材料**：`snapshot.rs` 全套原语、`LogRetentionSpec` 保留策略字段先例、
  `.supertask` 运行时目录。
- **验收雏形**：声明保留策略后超限快照自动清理且从不清除最新一份；定时触发点
  明确（引擎常驻事件循环）；关闭工作区不产生半截快照。
- **待细化**：spec 字段形态（`data.retention` vs 每卷字段）、定时器挂点与间隔、
...
### R. 环境快照上下文（结构化输出给 AI） —— 优先级：中

- **目标**：把工具版本、端口、健康、错误摘要聚合为一个「环境快照」结构化返回，
  AI 不靠多次调用拼凑上下文。
- **已有材料**：`Engine::diagnostics()` 聚合、`probe` 工具链探测、`host_metrics`、
  `metrics` 服务指标、MCP 出口统一脱敏。
- **验收雏形**：一次调用返回可直接进 prompt 的结构化上下文（全脱敏、大小有界、
  缺采样字段可区分）。
- **待细化**：字段矩阵与大小上限、与 `supertask_status` / `supertask_errors` 的
  取舍或合并。
...
> 以下对应本地规划 M2–M6（P0→P2），**本轮调整：M2 升为下一顺位**——CI 已稳定，
> 产物是「不承诺不可用平台」到「可验证分发」的直接下一步。

### T. M2 release 平台产物（macOS aarch64+x86_64 / Linux x86_64） —— 优先级：高（P0）

- **目标**：`release.yml` 增加 macOS / Linux 构建任务（不签名也可分发，标注
  Gatekeeper 放行方式）；更新检测端点已平台通用无需改。
- **验收雏形**：tag 触发产出三平台安装包并进 draft Release；CNB 镜像任务遍历
  所有 platform URL；README 平台表随产物更新。
- **待细化**：macOS 目标三元组与 universal2 取舍、Linux 产物形态（先 AppImage
  还是直接 tgz）、签名缺失时的分发文案。
