...
|---|---|---|---|---|
| 1 | T M2 release 平台产物 | 八 | **高**（P0） | M1 CI 已绿，发布产物是「对外承诺」的下一步；本地规划 M2=P0 |
| 2 | C Procfile 导入 | 二 | 中高 | 成本最小（格式极简）撬动 Overmind 用户池；preview/apply 模板现成 |
| 3 | F needs 钉扎写回 | 三 | 中高 | resolve 侧只读已交付，写回是纯增量小切片 |
| 4 | J 隧道公网 URL 提取 | 四 | 中 | 小成本高可感；复用日志管道 |
| 5 | R 环境快照上下文 | 七 | 中 | 聚合已有采样面，AI 价值高、无新权限 |
| 6 | O 工作区定时备份 | 六 | 中 | 快照归档机制已就绪，只差定时触发与保留 |
| 7 | A 运行中进程原地接管 | 二 | 中 | 叙事核心，但 Windows attach 权限边界需先调研 |
| 8 | M 隧道模板并入现有工作区 | 四 | 中 | 复用 preview/apply；与 C 同批可做 |
| 9 | G compose 作为 needs 来源 | 三 | 中 | 依赖方向二数据复用边界拍板 |
| 10 | E 归档供给执行器 | 三 | 中 | 打开能力上限但工程量大（下载器/校验/隔离） |
| 11 | V M4 三平台真机冒烟 | 八 | 中 | 需真机环境；K 网关行为清单并入本项 |
| 12 | D2 钩子 / 级联重启 / 失败保持 | 一 | 中 | ROADMAP 主表剩余 ★★ 项，打包成一个切片 |
