...
> **2026-08-29 已全部拍板**：① v1.7 = 三支柱组合；② Python/Go 提前至 1.7（插件/WSL2 留 2.2，generic 兜底「更多语言」）；③ 专用 kind + generic 同版；④ 探测 + 一键安装都做；⑤ 导出+导入都进 `/workspaces`（welcome 首启导入保留，settings 不留副本）；⑥ A1–A4 进本轮，B/C 单开验收专项。决策依据见 [v1.7 规格 §2.1–2.2](../archive/plans/2026-08-29-v1-7-feature-spec.md)。下表为决策过程存档。
