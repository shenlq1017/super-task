...
| macOS（Apple Silicon / Intel） | ✅ DMG（随 Release 发布，未签名） | ❌ | 实验性：Gatekeeper 首次运行需右键打开或 `xattr -cr` 放行；无真机验收 |
| Linux（x86_64） | ✅ AppImage / deb（随 Release 发布） | ❌ | 实验性：无真机验收 |

引擎代码含 Unix 分支（`proc/unix.rs`、Homebrew / sdkman / nvm 探测路径）；
macOS 与 Linux 已有三平台 CI 覆盖（编译 + 全量离线测试），安装包随 Release 产出；
因**未签名公证且未做真机验证**，标注为实验性，不作为可用平台对外承诺。
推进到可用所需的动作见 [未来考虑](#未来考虑) 中的「多平台可用」。
...
CI 在 Windows / macOS / Linux 三平台各自跑测试与格式化检查
（Linux 另装 Tauri 系统依赖做壳编译检查）。
...
- **多平台可用**：macOS 签名与公证、Linux 自动更新通道（AppImage）、三平台真机验收。
  三平台 CI 已覆盖（方向八 M1），三平台安装包已随 Release 产出（方向八 M2，未签名）。
