...
export function Input({ className, ...props }: React.ComponentProps<"input">) {
  return (
    <input
      className={cn(
        "flex h-9 w-full rounded-[var(--r-md,12px)] border border-[var(--line-strong,#d0d6e0)] bg-[var(--surface,#fff)] px-2.5 py-1 text-sm text-[var(--t1,#222326)] shadow-none outline-none transition-[border-color,box-shadow] duration-150 ease-[var(--st-ease,cubic-bezier(.22,1,.36,1))]",
