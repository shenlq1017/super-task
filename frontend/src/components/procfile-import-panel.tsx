/**
 * 方向二 Procfile 导入向导（ipc.md §10.19）。
 * 预览表 → 勾选 → 应用到 supertask.yaml；交互样式对齐 Taskfile 导入向导（taskfile-import-panel）。
...
import type { ProcfileImportItem, ProcfilePreview } from "@/ipc/protocol";
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          <input
            ...
            aria-label={t("pages.config.procfile.rowAria", { name: item.name })}
          />
          {greyed ? <Ban className="size-3.5 text-[var(--t3,#8a8f98)]" aria-hidden /> : null}
          {greyed ? t("pages.config.procfile.skipped") : t("pages.config.procfile.importAs")}
        </label>
        <span className="font-mono text-[0.82rem] font-semibold text-[var(--t1,#222326)]" title={item.name}>
          {item.name}
        </span>
        <ArrowRight className="size-3.5 shrink-0 text-[var(--t3,#8a8f98)]" aria-hidden />
        <span className="font-mono text-[0.78rem] text-[var(--st-accent,#5e6ad2)]">{item.service_id}</span>
        {item.id_conflict ? (
          <Badge variant="outline" className="border-[#f0d58a] bg-[#fdf6e3] text-[#B7791F]">
            {t("pages.config.procfile.conflictBadge")}
          </Badge>
        ) : null}
      </div>
      <div className="mt-1 truncate font-mono text-[0.72rem] text-[var(--t2,#62666d)]" title={item.command}>
        {item.command}
      </div>
...
export function ProcfileImportPanel({
  preview,
  checked,
  onToggle,
  onSelectAll,
  ...
  preview: ProcfilePreview;
  checked: Record<string, boolean>;
  onToggle: (serviceId: string, v: boolean) => void;
  ...
  const { t } = useTranslation();
  const selectable = preview.items.filter((it) => !it.skipped);
  const allChecked = selectable.length > 0 && selectable.every((it) => checked[it.service_id]);
  ...
  return (
    <section
      ...
      aria-label={t("pages.config.procfile.panelAria")}
    >
      <div className="flex shrink-0 flex-wrap items-center gap-2 rounded-t-[var(--r-lg,16px)] border-b border-[var(--line,#e6e6e6)] bg-[var(--st-accent-tint,#eef0fb)] px-3 py-2.5">
        <h3 className="text-[0.85rem] font-semibold text-[var(--t1,#222326)]">
          {t("pages.config.procfile.title")}
        </h3>
        <span className="font-mono text-[0.7rem] text-[var(--t3,#8a8f98)]">
          {t("pages.config.procfile.itemCount", { n: preview.items.length })}
        </span>
        {selectable.length > 0 ? (
          <Button size="sm" variant="outline" onClick={() => onSelectAll(!allChecked)}>
            {allChecked ? t("pages.config.procfile.unselectAll") : t("pages.config.procfile.selectAll", { n: selectable.length })}
          </Button>
        ) : null}
          ...
          <div className="flex flex-col gap-1.5">
            {preview.items.map((it) => (
              <ProcRow
                key={it.service_id}
                item={it}
                checked={checked[it.service_id] ?? false}
                onToggle={(v) => onToggle(it.service_id, v)}
              />
            ))}
          </div>
        )}
      ...
      <div className="flex shrink-0 items-center gap-2 rounded-b-[var(--r-lg,16px)] border-t border-[var(--line,#e6e6e6)] bg-[var(--surface-2,#f3f4f5)] px-3 py-2.5">
        <span className="min-w-0 flex-1 text-[0.74rem] text-[var(--t3,#8a8f98)]">
          {t("pages.config.procfile.applyHint")}
        </span>
        <Button size="sm" variant="default" onClick={onApply} disabled={applying || applyCount === 0}>
          {applying ? t("pages.config.procfile.applying") : t("pages.config.procfile.applySelected", { n: applyCount })}
        </Button>
      </div>
    </section>
  );
